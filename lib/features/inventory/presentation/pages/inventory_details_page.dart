  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: Colors.orange.shade700, // Or a color that suits your app
      ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (item.productId != null)
                          Text('Product ID: ${item.productId}', style: TextStyle(color: Colors.grey.shade600)),
                        if (item.quantityInStock != null)
                          Text('Stock: ${item.quantityInStock}', style: TextStyle(color: Colors.grey.shade600)),
                        if (item.lastUpdated != null)
                          Text('Last Updated: ${item.lastUpdated!.day}/${item.lastUpdated!.month}/${item.lastUpdated!.year}', style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                        if (item.location != null && item.location!.isNotEmpty)
                          Text('Location: ${item.location}', style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                        if (item.minimumStock != null)
                          Text('Min Stock: ${item.minimumStock}', style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
